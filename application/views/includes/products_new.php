  ...
  <?php
    ...
    if(!empty($products)){
      foreach ($products as $key) {
        if($key->seller_status==1 && $key->product_status==1 && $key->stock>0 && $key->new_product!=''){
          $total_records++;
        }
      }
    }
    ...
    for ($i=1; $i<=$total_pages; $i++) {
      if ($i==$pn) {
        $pagLink .= "<li class='active'><a href='".base_url()."page/new_products?page=".$i."'>".$i."</a></li>";
      }
      else  {
        $pagLink .= "<li><a href='".base_url()."page/new_products?page=".$i."'>".$i."</a></li>";
      }
    };
    echo $pagLink;
  ?>
